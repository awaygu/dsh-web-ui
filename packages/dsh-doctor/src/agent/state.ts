...
export interface PersistedState { phase: SupervisorSnapshot['phase']; profiles: Record<string, ProfileRuntime>; incidents: Record<string, IncidentRecord>; recentFailures: Record<string, string[]>; paused: boolean }
...
export function snapshotOf(state: PersistedState, version: string, now = new Date().toISOString()): SupervisorSnapshot {
  return { protocol: DOCTOR_PROTOCOL_VERSION, phase: state.phase, version, profiles: Object.values(state.profiles), incidents: Object.values(state.incidents).sort((a, b) => b.openedAt.localeCompare(a.openedAt)), updatedAt: now }
}
