...
`scripts/` contains repository maintenance tools, `docs/` contains long-lived documentation and archives, and `gallery/` contains the skin gallery site.
...
Before merging, run at least `pnpm typecheck && pnpm test && pnpm docs:check`. Run the aggregate, gallery, and Skin Center checks when those areas change.
